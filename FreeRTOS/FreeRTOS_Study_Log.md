...
#### 11.3 Debug相关的钩子（回调）函数

**分配失败钩**

定义一个malloc failed hook保证应用开发者能够在程序尝试创建任务，队列，信号量或事件组失败时，立即收到失败通知。在第2章节中有介绍，堆内存管理。

**堆栈溢出钩**

定义一个堆栈溢出钩保证应用开发者能够在任务使用的堆栈空间超过了分配给它的堆栈空间大小时，立即收到溢出通知。在12.3节中将详细介绍堆栈溢出钩。

#### 11.4 查看Run-time和任务状态信息

**任务运行时间统计**

任务运行时间统计提供了每个任务获得的总的处理时间的信息。一个任务的运行时间是从应用启动之后处于运行态的时间总和。

在项目的开发阶段，运行时间统计经常被用作分析和调试的目的。它们提供的信息尽在作为运行时间统计时钟使用的计数器已销户之前有效。收集运行时间统计将会增加任务上下文切换的时间。

调用uxTaskGetSystemState()API函数可以获取运行时间统计的二进制信息。调用vTaskGetRunTimeStats()辅助函数可以获取可读的运行时间统计信息，以ASCII表的方式。
...
每一个trace宏都默认有一个空定义。默认定义的这些宏可以被新的宏重写，这些宏在FreeRTOSConfig.h文件中定义。如果trace宏的定义变得又长又复杂，那么可以将这些宏的实现放到一个新的头文件中，然后将这个头文件包含到FreeRTOSConfig.h文件中。

根据软件工程的最佳实践，TreeRTOS维护严格的数据隐藏策略。trace宏允许用户代码添加到FreeRTOS的源代码中，因此对trace宏可见的数据结构将不同于应用代码可见的数据类型：

- 在FreeRTOS/Source/task.c源文件中，任务句柄是一个指向描述任务数据结构（任务的任务控制块，TCB）的指针。而在FreeRTOS/Source/task.c源文件的尾部，任务句柄是一个指向void的指针。
- 在FreeRTOS/Source/queue.c源文件中，一个队列句柄是一个指向描述队列数据结构的指针。然，在FreeRTOS/Source/queue.c源文件的外部是一个指向void的指针。

***如果私有的FreeRTOS数据结构被一个trace宏直接访问时，需要格外的小心，因为在不同的FreeRTOS版本之间这些私有的数据结构可能会有所改变。***

### 12 Trouble Shooting

#### 12.1 中断优先级

注意：这是最容易出错的地方，在大多数端口中都会定义configASSERT()去及时捕获这个错误。

如果使用FreeRTOS的端口支持中断嵌套，并且中断的服务程序中使用了FreeRTOS API函数，那么这个中断的优先级必须设置为等于或者小于configMAX_SYSCALL_INTERRUPT_PRIORITY，如6.8节Interrupt Nesting中所述。如果不这样做，将导致无效的临界区，这将导致间歇性的失败。

如果在处理器上运行FreeRTOS需要尤其注意以下地方：

- 在ARM Cortex或者其他的一些处理器上，中断的优先级默认是最高的。在这些处理器上，使用FreeRTOS API函数的中断优先级必须进行初始化。
- 数值越大表示的优先级越低，这看起来违反直觉，因此可能会使人迷惑。这种情况也会出现在ARM Cortex或者其他的一些处理器上。
- 例如，在一个处理器上，一个中断优先级是5的中断可以被一个优先级为4的中断中断。因此，如果设置**configMAX_SYSCALL_INTERRUPT_PRIORITY**等于5，那么任何使用FreeRTOS API函数的中断只能被分配一个数值上大于或等于5的优先级。在这种情况下，中断优先级为5或者6的中断是有效的，而优先级为3或4的是无效的。
- 不同的库实现期望以不同的方式指定中断的优先级。与针对ARM Cortex的库相关的处理器，在被写入硬件寄存器之前要进行优先级位移。一些库本身会执行这个位移操作，然而其他的一些库则希望优先级在输入库函数之前进行优先级的位移操作。
- 相同架构处理器的不同实现，其中断优先级的位数也不相同。例如同样的Cortex-M处理器，不同的厂家有不同的优先级位数实现，一个可能是3位，另一个可能是4位。
- 定义中断优先级的位可以被划分位定义优先级的位和定义次优先级的位。确保分配所有的位来指定一个优先级，这样就不会使用子优先级。

在一些FreeRTOS端口中，configMAX_SYSCALL_INTERRUPT_PRIORITY有一个别名叫configMAX_API_CALL_INTERRUPT_PRIORITY。

#### 12.2 堆栈溢出

堆栈溢出是排名第二的常见问题。FreeRTOS中提供了一些特性辅助捕获并调试与堆栈相关的问题。（在FreeRTOS的Windows端口，这些特征不可用）

**uxTaskGetStackHighWaterMark() API函数**

每一个任务都维护了一个自己的堆栈，任务使用堆栈的大小在其创建时指定。uxTaskGetStackHighWaterMark()函数用来询问任务最接近堆栈溢出的程度。这个值被称作堆栈的“高水位”。函数原型如下：

![image-20201210175021484](illustration/image-20201210175021484.png)

**参数：**

xTask：需要被询问堆栈高水位的任务的句柄-要获取任务句柄可参见xTaskCreate() API函数的pxCreatedTask参数的信息。通过给xTask参数传入NULL可以查询任务自身的堆栈高水位。

**返回值：**

随着任务的执行和中断的处理，任务使用的堆栈数量会增加或者减少。uxTaskGetStackHighWaterMark()函数返回自任务开始执行到调用函数时剩余可用堆栈空间的最小值。也就是当堆栈使用量最大的时候，剩余的可使用的堆栈空间大小。堆栈的高水位越接近0，这个任务就越有可能发生堆栈溢出。

**运行时堆栈检查**

FreeRTOS包含有两种运行时堆栈检查的方案。这两种方案由编译时配置给FreeRTOS.h头文件中configCHECK_FOR_STACK_OVERFLOW常量的参数所控制。两种方案都会增加执行上下文切换所需的时间。

堆栈溢出钩子（或堆栈溢出回调函数）是在检测到堆栈溢出时由内核调用的函数。要使用堆栈溢出钩子函数：

1. 需要在FreeRTOSConfig.h头文件中设置configCHECK_FOR_STACK_OVERFLOW为1或者2，这将在下面的小节中介绍。
2. 需要使用下图要求的函数名及函数原型提供钩子函数的实现。

![image-20201210182604073](illustration/image-20201210182604073.png)

提供的堆栈溢出钩子函数可以时堆栈错误更容易被捕获及调式，但是当堆栈溢出发生时，没有办法从堆栈溢出错误中恢复。发生堆栈溢出任务的句柄和任务名通过函数的参数传递给钩子函数。

从中断的上下文中调用堆栈溢出钩子。

一些微控制器在检测到不正确的内存访问时产生一个故障异常，并且有可能在内核有机会调用堆栈溢出钩子函数之前触发故障。

**运行时堆栈检查-方法1**

当设置configCHECK_FOR_STACK_OVERFLOW为1时，选择方法1。

当任务每一次切出运行态时，将所有的执行上下文存入其堆栈。这几乎是任务使用其堆栈最大的时刻。当configCHECK_FOR_STACK_OVERFLOW设置为1时，在上下文全部保存完毕之后，内核检查堆栈指针是否保留在有效的堆栈空间内。如果发现堆栈指针在有效范围之外，将调用堆栈溢出钩子函数。

方法1执行的很快，但是会漏掉发生在上下文切换时的堆栈溢出。

**运行时堆栈检查-方法2**

方法2在方法1的基础上增加了额外的检查。当configCHECK_FOR_STACK_OVERFLOW设置为2时，使用方法2。

当任务被创建时，它的堆栈全部填充为一个已知的模式。方法2检查任务堆栈空间的最后有效的20个字节空间，验证这个模式没有被覆盖。如果这20个字节中的任何一个字节从其已知的模式发生改变，则调用堆栈溢出函数。

方法2的执行虽不如方法1快，但仅仅测试20个字节，其执行速度也很快。方法2最有可能够捕获所有堆栈溢出的发生；然而，它也有可能漏掉溢出的放生（几乎没可能）。

#### 12.3 不适当的使用printf() & sprintf()

错误的使用printf()函数是一个常见的错误，由于对它不了解，通常应用开发人员为了调试会添加很多printf()函数的调用，如此一来，就产生了问题。

许多交叉编译供应商都会提供适合在小型嵌入式系统中使用的printf()函数的实现。即使是这样，那些实现可能不是线程安全的，很可能不适合在中断服务程序内部调用它，且其执行时间直接取决于输出到哪里，可能执行时间较长。

必须特别小心，如果一个专为小型嵌入式系统设计的printf()函数不能使用，那么将会使用普通的printf()函数，此时：

- 仅仅包含对printf()或sprintf()的调用就可以极大地增加应用程序可执行文件的大小。
- printf()和sprintf()函数可能调用malloc()函数，如果内存分配机制不是使用heap_3，malloc()是不可用的。参看2.2节Example Memory Allocation Schemes，查看更多详情。
- printf()和sprintf()可能需要一个比其他情况下需要大几倍的堆栈。

**Printf-stdarg.c**

大部分的FreeRTOS演示工程使用一个名为printf-stdarg.c的源文件，这个文件提供了一个小型的且能有效使用堆栈的sprintf()函数实现，使用它来代替标准库中的sprinf()版本。在大多数情况下，这个文件中的sprintf()函数允许为调用它或其相关函数的每个任务分配更小的内存。

printf-stdarg.c还提供了一种机制，用于将printf()输出逐个字符地定向到端口，虽然慢，但大大降低了堆栈的使用。

注意到在FreeRTOS的下载中不是所有的printf-stdarg.c文件都有snprintf()函数的实现。没有snprintf()函数实现的副本中可以直接忽略buffer size参数，直接映射到sprintf()函数。

Printf-stdarg.c是开源的，但是其由第三方拥有，因此其与FreeRTOS分开授权。其许可证包含在源文件的顶部。

#### 12.4 其他常见错误

**现象：向演示项目中添加简单的任务导致演示崩溃**

创建任务需要从堆内存中获取内存。多数的演示程序项目将堆的尺寸确定为足以创建演示任务的大小-因此，在这个任务创建之后，剩下的堆内存可能不足以供创建其他任务、队列、事件组或信号量的创建。

除了空闲任务，RTOS守护进程任务也有可能在调用vTaskStartScheduler()函数时自动创建。只有当剩余的堆内存红箭不足以供这些任务创建时vTaskStartScheduler()函数才会返回。在调用vTaskStartSchedler()函数后面添加一个空循环[for(;;);]可以使这个错误更容易调试。

要想添加更多的任务，增加堆内存的尺寸，或者删除一些存在的演示任务。可以查看2.2节Example Memory Allocation Schemes获取更多信息。

**现象：在中断内部使用API函数导致应用崩溃**

如果API函数名不是以‘FromISR()’结尾的话，千万不要在中断服务程序中使用。特别的，如果使用的不是中断安全宏，万万不要再中断中创建临界区。查看6.2节Using the FreeRTOS API from an ISR获取更多详情。

在支持中断嵌套的FreeRTOS端口中，不要再中断优先级大于configMAX_SYSCALL_INTERRUPT_PRIORITY的中断中使用任何的API函数。看6.8节Interrupt Nesting获取更多详情。

**现象：中断服务程序中的应用崩溃**

这种情况首先确保中断不会导致堆栈溢出。一些FreeRTOS端口只在任务中检查堆栈是否会溢出，不在中断中检查。

端口之间以及编译器之间定义和使用中断的方式不同。因此，其次要检查的时中断服务程序中所使用的语法、宏和调用约定是否符合所使用端口提供的文档页面描述的那样，并完全如端口随附的演示用用程序中所示。

如果运行应用的处理器使用数值上低的数字代表逻辑上高的优先级，那么需确保给每一个中断分配优先级的时候，注意到这一点，因为这样的优先级表示确实反直觉。如果运行程序的处理器默认每一个中断的优先级都是最高的，那么确保每一个中断的优先级不是其默认值，即要给中断优先级初始化。查看6.8节Interrupt Nestingh和12.2节Interrupt Priorities获取更多信息。

**现象：当尝试运行第一个任务时，调度器崩溃**

确保安装了FreeRTOS的中断处理程序。查看所使用FreeRTOS端口的文档页面获取信息，使用提供的示例应用作为例子。

一些处理器必须进入特权模式才能启动调度器。实现这种情况最简单的办法就是在调用main()主函数之前，在C启动代码中将处理器进入特权模式。

**现象：中断被意外禁用，或者临界区嵌套错误**

如果一个FreeRTOS API函数在调度器开始之前被调用，那么中断将会被故意禁用，直到第一个任务开始执行时都不会将其激活。这样做是为了保护系统免遭系统初始化期间，启动调度程序之前以及调度器程序可能处于不一致状态期间因尝试使用FreeRTOS API函数的中断而导致的崩溃。

如果没有调用taskENTER_CRITICAL()和taskEXIT_CRITICAL()两个函数，不要使用任何方法改变微控制器的中断使能位或者优先级标志。这些宏对它们的调用嵌套深度进行计数，以确保只有在调用嵌套完全解除到零时才再次启用中断。请注意，一些库函数本身可能启用和禁用中断。

**现象：在调度器启动之前，应用程序崩溃**

在调度器启动之前，不允许执行可能导致上下文切换的中断服务程序。不允许执行的还有任何尝试发送到FreeRTOS对象或从FreeRTOS对象接收的中断服务程序，FreeRTOS对象包括队列、信号量等。直到调度器启动，任何上下文切换都不允许发生。

许多API函数都不能被调用，直到调度器启动之后。在vTaskStartScheduler()函数调用之前，最好将API的使用限制在仅创建诸如任务，队列和信号量之类的对象，而不是使用这些对象。

**现象：在调度器被挂起时或在临界区内调用API函数，导致应用崩溃**

通过调用vTaskSuspendAll()函数来挂起调度器，调用xTaskResumeAll()函数恢复调度器的使用。调用taskENTER_CRITICAL()函数进入临界区，调用taskEXIT_CRITICAL()函数退出临界区。

不要在调度器被挂起时，或者在临界区域内调用API函数。
