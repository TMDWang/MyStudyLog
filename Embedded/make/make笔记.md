...
- 显式规则：显式规则说明了如何生成一个或多个目标文件。由Makefile的书写着明显指出，要生成的文件、文件的依赖文件、生成的命令。
- 隐晦规则：由于make有自动推导的功能，所以隐晦的规则可以让我们比较粗糙地简略地Makefile，这是由make所支持的。
...
通常，如果一个依赖发生了变化，就需要规则调用命令以更新或创建目标。但是并非所有的目标都有依赖，例如，目标“clean”的作用是清楚文件，它没有依赖。
...
在Makefile中经常看到=、:=、?=、+=这几个赋值运算符。他们的区别用线面的例子展示
...
make DEFINE_VRE=true OPT=add  # 输出：
make DEFINE_VRE= OPT=add  # 输出：
make DEFINE_VRE=true OPT=recover  # 输出：
make DEFINE_VRE= OPT=recover  # 输出：
```
